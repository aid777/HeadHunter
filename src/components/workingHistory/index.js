...
export default function WorkingHistory({workingHistory, remove}){
    ...
    return(
        <div className="working-history">
            <p>{monthsInRussian[startDate.getMonth()]} {startDate.getUTCFullYear()} - {monthsInRussian[endDate.getMonth()]} {endDate.getUTCFullYear()}</p>
            <h4>{workingHistory.company_name}</h4>
            <p>{workingHistory.company_description}</p>
            <span onClick={() => remove(workingHistory)}></span>
        </div>
    )
}